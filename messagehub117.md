...
Message Hub uses throughput quotas to control the network bandwidth used by clients. This is to minimise the impact of clients on each other.

Quotas work by measuring the throughput of producers and consumers, and then throttling those that exceed the quotas by slightly delaying the responses to their requests. The effect is to apply a gently brake to producers and consumers which attempt to consume a lot of bandwidth.

Message Hub assigns a throughput quota to each Message Hub service instance. A separate quota is used for producers and consumers. The quota is proportional to the number of partitions created for that service instance, although it is not applied per partition.

An example will help illustrate. Consider a service instance with 10 topics, each with 1 partition. The throughput quota for producers is:
10 x 5 MB/s = 50 MB/s
The quota is applied across the nodes of the Kafka cluster
and the precise value applied at a point in 
time depends on the current distribution of partition leader
s on the nodes of the cluster.
