...
{: #ui}
Message audit events can be enabled on a per topic basis. To do so, completet the follwing steps:
...
1. An internal Kafka topic is used for streaming events to {{site.data.keyword.cloudaccesstrailshort}}, thus it uses a small amount of the cluster's network bandwidth and storage. Typically throughput is less than 1KB/s, and storage is less than 100 MB/day.
