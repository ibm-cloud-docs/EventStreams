...
Many configuration settings exist for the consumer, which control aspects of its behavior. Some of the most important settings are the following:
...
If the batch of messages that are returned from `Consumer.poll(...)` is large or the processing is time-consuming, 
the delay before calling `poll()` again can be significant or unpredictable. In some cases, it's necessary to configure a long maximum polling interval 
so that consumers do not get removed from their groups just because message processing is taking a while. 
If this was the only mechanism, the time taken to detect a failed consumer would also be long.
...
the consume rate should not be slower than the produce rate for an extended period.
