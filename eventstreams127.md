...
The endpoints in service key are geneated according to the service endpoints settings of the instance. eg. public endpoints are genearted for `--service-endpoints=public`, private endpoints are generated for `--service-endpoints=private`. However for instance that has `--service-endpoints=public-and-private`, public endpoints are generated by default. To generate private endpoints in service key, use command: <code>ibmcloud resource service-key-create <private-key-name> <role> --instance-name <instance-name> --service-endpoint private<code> .
{:important}
